function make_decoder_figure(phys_errors, results;
    ...
    Legend(f[1,4],ca,colorlabels, "Code", framevisible = false, halign=:left, titlehalign=:left, valign=:top)
    la = []
    for (iᵈ,label) in enumerate(linestylelabels)
        push!(la, lines!(a, [NaN], [NaN], linestyle=linestyles[iᵈ], color=:gray, label=label))
    end
    Legend(f[2,4],la,linestylelabels, "Decoder", framevisible = false, halign=:left, titlehalign=:left, valign=:top)
    ma = []
    if single_error
        for (iˢ,label) in enumerate(markerlabels)
            push!(ma, scatter!(a, [NaN], [NaN], marker=markers[iˢ], color=:gray, label=label))
        end
        Legend(f[3,4],ma,markerlabels, "Circuit Type", framevisible = false, halign=:left, titlehalign=:left, valign=:top)
    else
        push!(ma, scatter!(a, [NaN], [NaN], marker=:+, color=:gray, label="X"))
        push!(ma, scatter!(a, [NaN], [NaN], marker=:x, color=:gray, label="Z"))
        Legend(f[3,4],ma,["X", "Z"], "Logical Error", framevisible = false, halign=:left, titlehalign=:left, valign=:top)
    end
    f
end
